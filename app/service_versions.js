...
function showDataLossModal(opts) {
  ...
  return new Promise((resolve) => {
    ...
    p.textContent =
      'Changing versions can reset your service data. Choose one option to continue, or cancel.';
    ...
    createVersionRow(v, {
      ...
      renderActions: (vv) => {
        ...
        const canSync =
          ...
          vv.matchHint === 'Different from the published version' &&
          !isOffline;
...
async function loadVersions() {
  const api = window.serviceVersionsAPI;
  if (!api) {
    setBanner('error', 'Service Versions API is not available.');
    return;
  }
