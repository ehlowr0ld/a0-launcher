...
1. Load the existing constitution template at `.specify/memory/constitution.md`.
   ...
   **IMPORTANT**: The user might require fewer or more principles than the ones used in the template. If a number is specified, respect that and adapt section counts accordingly while preserving the template's structure and headings.
...
4. Consistency propagation checklist (active validations):
   ...
   - Read each command file in `.cursor/commands/*.md` (including this one) to verify no outdated agent-specific references remain where generic guidance is appropriate.
