...
async function buildDerivedState(options = {}) {
  ...
  // First-class preview/testing entry (not derived from GitHub Releases).
  {
    ...
    let matchHint = null;
    if (localDigest && cacheEntry && typeof cacheEntry.digest === 'string' && cacheEntry.digest) {
      if (localDigest !== cacheEntry.digest) {
        matchHint = 'Different from Testing preview';
      }
    }
    versions.push({
      id: tag,
  ...
  for (const r of releases) {
    ...
    if (localDigest && cacheEntry && typeof cacheEntry.digest === 'string' && cacheEntry.digest) {
      if (localDigest !== cacheEntry.digest) {
        matchHint = 'Different from the official version';
      }
    }
  ...
  for (const tag of localBuildTags) {
    ...
    if (isCanonicalLocalTag(tag)) {
      if (cacheEntry?.status === 'installable') {
        ...
        if (localDigest && typeof cacheEntry.digest === 'string' && cacheEntry.digest) {
          if (localDigest === cacheEntry.digest) {
            matchHint = 'Matches the published version';
          } else {
            matchHint = 'Different from the published version';
          }
        }
    ...
    if (!matchHint && localDigest) {
      ...
      if (match && isSemverReleaseTag(match.tag)) {
        matchHint = `Matches official version ${match.tag.startsWith('v') ? match.tag.slice(1) : match.tag}`;
        ...
        matchHint = 'Matches Testing preview';
      }
    }
